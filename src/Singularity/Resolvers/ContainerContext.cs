...
{
    internal sealed class ContainerContext : IContainerContext
    {
        ...
        private readonly ContainerContext? _parentPipeline;
        private readonly Scoped _containerScope;
        private readonly ExpressionGenerator _expressionGenerator = new ExpressionGenerator();

        public ContainerContext(RegistrationStore registrationStore, Scoped containerScope, SingularitySettings settings, ContainerContext? parentPipeline)
        {
        ...
        private Registration? TryGetDependency(Type type)
        {
            lock (SyncRoot)
            {
                ...
                foreach (IServiceBindingGenerator dependencyResolver in _resolvers)
                {
                    ...
                    ServiceBinding[] serviceBindings = dependencyResolver.TryGenerate(this, type).ToArray();
